def timeline_from_objects(objects: list[dict], chunks_by_id: dict[str, dict]) -> list[dict]:
    ...
    for item in objects:
        ...
        if chunk:
            timeline.append({"id": item["id"], "date": item["data"].get("date"), "time": item["data"].get("time"),
    ...
    return sorted(timeline, key=lambda item: (item.get("date") or "9999", item.get("time") or "99:99"))
