...
export function OverviewView({ caseId, status, onCitation, onTab }: { caseId: string; status?: { counts: Record<string, number | string> }; onCitation: (value: Citation) => void; onTab: (value: CaseTab) => void }) {
  ...
  return <div className="mx-auto max-w-[1320px]">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
      <section className="panel p-6"><div className="flex items-center justify-between"><div><div className="eyebrow">Verified workspace synthesis</div><h2 className="mt-1.5 text-lg font-semibold">Case summary</h2></div><Scale size={20} className="text-teal-700" /></div><p className="mt-5 text-sm leading-7 text-slate-600">{data.summary}</p><div className="mt-5 rounded-lg border border-amber-100 bg-amber-50/60 p-3 text-xs leading-5 text-amber-800">This system supports evidence review. It does not determine guilt, innocence, or likely case outcome.</div></section>
