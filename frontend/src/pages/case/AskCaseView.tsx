...
export function AskCaseView({ caseId, onCitation, readOnly = false }: { caseId: string; onCitation: (value: Citation) => void; readOnly?: boolean }) {
  const { t } = useLanguage()
  ...
  const query = useMutation({
    ...
    onError: (reason, variables) => setMessages(current => current.map(message => message.id === variables.id ? { ...message, error: reason instanceof Error ? reason.message : t('The request failed. Please try again.'), pending: false } : message)),
  })
