...
import { AlertCircle, CheckCircle2, FileText, LockKeyhole, UploadCloud, X } from 'lucide-react'
...
export function NewAnalysis({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const select = (candidate?: File) => {
    ...
    if (candidate.size > 250 * 1024 * 1024) return setError('The PDF exceeds the 250 MB limit.')
    setFile(candidate)
  }
  const submit = async () => {
    if (!file || !caseNumber.trim()) return setError('Case ID and a PDF are required.')
    ...
    try {
      ...
      await api.upload(created.id, file, role)
