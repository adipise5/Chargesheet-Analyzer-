    ...
    <aside className="app-sidebar fixed inset-y-0 left-0 z-20 flex w-[258px] flex-col bg-navy text-slate-100">
      <div className="sidebar-brand border-b border-white/10 px-5 py-5">
        <div className="flex items-center gap-3">
          ...
          <div><div className="text-[11px] font-semibold uppercase tracking-[.13em] text-slate-400">{t('Gujarat Police')}</div><div className="mt-0.5 text-sm font-semibold leading-tight">{t('Chargesheet Intelligence')}</div></div>
        </div>
      </div>
      <nav className="sidebar-nav space-y-1 px-3 py-4" aria-label="Primary">
        ...
        <NavButton active={screen === 'new'} icon={<FilePlus2 size={17} />} label={t('New Analysis')} onClick={onNew} />
      </nav>
      ...
      <div className="sidebar-footer space-y-1 border-t border-white/10 p-3">
        ...
        <button onClick={onPurge} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-red-300 transition hover:bg-red-950/40 hover:text-red-200"><Trash2 size={15} /> {t('Purge sensitive data')}</button>
        <div className="mt-3 flex items-start gap-2 rounded-lg bg-emerald-950/35 px-3 py-3 text-[10px] leading-relaxed text-emerald-100/75"><LockKeyhole className="mt-0.5 shrink-0" size={13} />{t('On-device processing. No case content leaves approved local services.')}</div>
      </div>
    </aside>
  )
}
